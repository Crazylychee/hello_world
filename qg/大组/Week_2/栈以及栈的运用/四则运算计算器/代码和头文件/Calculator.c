...
int transform(char *str)//使用const保护数据,函数用来将中缀转换成后缀
{
    ...
    while(str[i]!='\0')//遍历整个字符串
    {
        ...
        else if(is_operator((str[i]))==1)
        {
            ...
            if(isempty(istack)==0)//栈不为空
            {
                while((isempty(istack)==0)&&(priority(str[i])<=(priority(seqstack_top(istack)->value))))//栈不为空并且新运算符优先级不高于栈顶
                {
                    char_put(seqstack_pop(istack)->value);//满足条件的栈顶就弹出直到不满足条件
                    char_put(' ');
                }
            }
        ...
        {
            while(is_left(seqstack_top(istack)->value)!=1)//栈顶不是左括号的情况
            {
                char_put(seqstack_pop(istack)->value);//弹出并存储到结果串
    ...
    if(str[i]=='\0')//成功遍历到字符串末尾
    {
        ...
        {
            if(seqstack_top(istack)->value =='(')//栈顶元素为左括号
            {
                printf("有没有匹配到的'(',缺少')'\n");
                return -1;
            }
            char_put(seqstack_pop(istack)->value);//将栈中元素放入最终串
        }
    }
    else
    {
